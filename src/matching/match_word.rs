...
pub fn matches_single<T: Eq>(
    ...
    if remaining_word <= max_deletions && remaining_candidate <= max_insertions {
        Some(max_deletions - remaining_word + max_insertions - remaining_candidate + max_substitutions)
    } else {
        None
    }
}
...
#[inline(always)]
pub fn matches_deletion_only(word: &[u8], candidate: &[u8], mut max_deletions: usize) -> Option<usize> {
    let wlen = word.len();
    let clen = candidate.len();
    ...
    Some(max_deletions - (wlen - wi))
}

/// Checks if `candidate` can be formed by inserting at most `max_insertions` bytes into `word`.
#[inline(always)]
pub fn matches_insertion_only(word: &[u8], candidate: &[u8], max_insertions: usize) -> Option<usize> {
...
mod matching {
    ...
    #[test]
    fn single() {
        assert_eq!(matches_single("traanslatin".as_bytes(), "translation".as_bytes(), 2, 2, 0), Some(2));
    }

    #[test]
    fn only_deletions() {
        assert_eq!(matches_deletion_only("transglagtioon".as_bytes(), "translation".as_bytes(), 3), Some(0));
    }

    #[test]
    fn only_insertions() {
        assert_eq!(matches_insertion_only("tranlain".as_bytes(), "translation".as_bytes(), 3), Some(0));
    }
}
